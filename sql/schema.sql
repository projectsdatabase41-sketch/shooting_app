...
-- тебе писал, когда разбирал ту серию". Одна строка — один обмен
-- вопрос-ответ (не сырая переписка целиком, а короткая выдержка —
-- см. AiChatViewModel._gist), о каких тренировках шла речь (может
-- быть ни одной — общий разговор). Ищется по ключевым словам вопроса
-- (AiMemoryService.search), а не отдаётся слепым списком "последние
-- N" — иначе в контекст лез бы разговор совсем не в тему. Пишется и
-- читается приложением (`AiMemoryService`), не руками.
...
-- ============================================================
-- Список таблиц личной базы — для "Таблицы для ИИ" в настройках
-- учётной записи (пункт 3/8 списка правок). RPC вместо запроса
-- OpenAPI-описания на /rest/v1/ напрямую: у части проектов корневой
-- путь PostgREST отвечает 401 (проверено пользователем на практике),
-- а обычный вызов функции идёт через тот же путь и те же заголовки,
-- что и любой другой запрос в приложении, и работает гарантированно.
-- ============================================================
