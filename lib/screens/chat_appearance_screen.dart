    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          _colorTile('Мой пузырь', prefs.mineBubbleColor, (c) => prefs.mineBubbleColor = c),
          _colorTile('Пузырь собеседника', prefs.otherBubbleColor, (c) => prefs.otherBubbleColor = c),
    ...
    try {
      ...
      final reply = await ai.ask(
        systemPrompt: 'Ты помогаешь настроить ВНЕШНИЙ ВИД чата в приложении для стрелкового спорта: '
            ...
            'Если пользователь просит что-то изменить — коротко подтверди своими словами, что делаешь, '
            'и в конце ответа добавь блок ```chat_theme с ТОЛЬКО теми полями, которые нужно поменять '
