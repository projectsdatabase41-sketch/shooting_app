...
class _SpecChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (bar) {
      ...
      // Линия — не столбик, ноль ей не обязателен: результаты стрельбы
      // обычно плотно сбиты в узком диапазоне (9.8–10.5), и настоящая
      // ось от нуля превращает линию в почти прямую, где не видно
      // никакой разницы (решение пользователя: "график слишком ровный",
      // "пусть хотя бы -2 делает"). Вместо нуля — запас вокруг САМИХ
      // данных, по 15% диапазона с каждой стороны; на почти плоской
      // линии (диапазон ~0) — фиксированный запас, чтобы не расплющило.
